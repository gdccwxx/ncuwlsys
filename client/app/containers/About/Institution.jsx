        ...
        <div className='container'>
          ...
          <div className='row'>
            <div className='col-md-3'><div className='list-group'>
              ...
              <Link to='/about/institution' className={window.location.pathname === '/about/institution' ? 'list-group-item select' : 'list-group-item'} >机构设置</Link>
              <Link to='/about/rules' className={window.location.pathname === '/about/rules' ? 'list-group-item select' : 'list-group-item'} >规章制度</Link>
            </div></div>
      ...
      <div className='list-group'>
        {
          data.map(item => {
            return (
              <Link to={'/article/' + item.id} className='list-group-item'><i className='fa fa-file-text-o' /> <strong>{item.title}</strong> <small className='text-f'> <i className='fa fa-clock-o fa-fw' /> {item.createtime}</small></Link>
            )
          })
        }
      </div>
...
module.exports = withRouter(Institution)
